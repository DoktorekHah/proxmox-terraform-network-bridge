...
test-basic: ## Run basic bridge test
	@echo "$(BLUE)Running basic bridge test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgeBasic
	@echo "$(GREEN)✓ Basic test completed$(NC)"

test-multiple: ## Run multiple bridges test
	@echo "$(BLUE)Running multiple bridges test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgeMultiple
	@echo "$(GREEN)✓ Multiple bridges test completed$(NC)"

test-ipv6: ## Run IPv6 bridge test
	@echo "$(BLUE)Running IPv6 bridge test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgeWithIPv6
	@echo "$(GREEN)✓ IPv6 test completed$(NC)"

test-vlan: ## Run VLAN-aware bridge test
	@echo "$(BLUE)Running VLAN-aware bridge test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgeWithVLAN
	@echo "$(GREEN)✓ VLAN test completed$(NC)"

test-plan: ## Run plan-only test
	@echo "$(BLUE)Running plan-only test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgePlanOnly
	@echo "$(GREEN)✓ Plan-only test completed$(NC)"

test-validate: ## Run validation test
	@echo "$(BLUE)Running validation test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkBridgeValidation
...
terratest-init: ## Initialize terratest Go module
	...
	cd terratest && go mod init github.com/proxmox/modules/proxmox-terraform-network-bridge/terratest || true
